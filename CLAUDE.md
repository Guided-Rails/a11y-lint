...
- **CLI:** `lib/a11y/lint/cli.rb` — command-line interface; executable at `exe/a11y-lint`; routes `.erb` files to `ErbRunner` and `.rb` files to `PhlexRunner`
...
- **Ruby code parsing:** `RubyCode` (`lib/a11y/lint/ruby_code.rb`) — parses a Ruby code string once with Prism and exposes the resulting `CallNode`; used by `ErbNode` so rules work against a parsed AST instead of re-parsing strings
...
- **Dummy app:** `test/fixtures/dummy_app/` — a fixture app with ERB/Phlex templates for end-to-end smoke testing before releases (`bundle exec a11y-lint test/fixtures/dummy_app`)
