...
a11y-lint supports ERB and Phlex. When a rule page shows example code, render it through the `Shared::CodeTabs` component so both pipelines collapse into one tabbed block instead of two stacked code blocks.

ERB is required and is the default tab. Phlex is optional — omit the kwarg and the tab won't render.
...
- Rouge syntax highlighting per tab (using the `erb` and `ruby` lexers).
